 ...
 */
package org.foi.nwtis.zorhrncic.web.filter;
...
public class AppLogFilter implements Filter {
    ...
    public AppLogFilter() {
        System.out.println("initlog fil");
    }

    private void doBeforeProcessing(ServletRequest request, ServletResponse response)
            throws IOException, ServletException {
        if (debug) {
            log("AppLogFilter:DoBeforeProcessing");
        }
    ...
    private void doAfterProcessing(ServletRequest request, ServletResponse response)
            throws IOException, ServletException {
        if (debug) {
            log("AppLogFilter:DoAfterProcessing");
        }
    ...
    public void init(FilterConfig filterConfig) {
        ...
        if (filterConfig != null) {
            if (debug) {
                log("AppLogFilter:Initializing filter");
            }
        }
    }
    ...
    public String toString() {
        ...
        StringBuffer sb = new StringBuffer("AppLogFilter(");
    ...
    private void saveDataToLog(ServletRequest request, ServletResponse response, long startTime, long elapsed) {
        ...
        System.out.println("save data to log");
        log("url: " + url);
        log("remote: " + remoteAddr);

        log("start [" + no + "]: " + (startTime) + " ms");
        log("trajanje [" + no + "]:  " + (elapsed) + " ms");
        Dnevnik log = new Dnevnik();
        log.setIpadresa(remoteAddr);
        log.setKorisnik(((HttpServletRequest) request).getSession().getId());
        log.setStatus(1);
        log.setTrajanje((int) elapsed);;
        log.setUrl(url);
